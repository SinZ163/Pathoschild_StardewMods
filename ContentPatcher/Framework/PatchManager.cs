    ...
    internal class PatchManager
    {
        ...
        public void UpdateContext(IGameContentHelper contentHelper, IInvariantSet globalChangedTokens, ContextUpdateType updateType)
        {
            ...
            while (patchQueue.Any())
            {
                ...
                try
                {
                    ...
                    changed = patch.UpdateContext(tokenContext, ref writer);
                }
                catch (Exception ex)
                {
     ...
     reloadAssets:          {(sw.Elapsed.TotalMilliseconds - timerVerboseLogging.TotalMilliseconds):N}ms");
        ...
        ****/
        ...
        public void Add(IPatch patch, ref IndentedTextWriter diagnostics, bool reindex = true)
        {
            var sw = Stopwatch.StartNew();
            // set initial context
            ModTokenContext modContext = this.TokenManager.TrackLocalTokens(patch.ContentPack);
            var timerTrackTokens = new TimeSpan(sw.ElapsedTicks);
            var writer = new IndentedTextWriter();
            writer.Indent = diagnostics.Indent + 1;
            patch.UpdateContext(modContext, ref writer);
            var timerUpdateContext = new TimeSpan(sw.ElapsedTicks);
            ...
            sw.Stop();
            diagnostics.WriteLine($"PatchManager.Add took {sw.Elapsed.TotalMilliseconds:N}ms and reindex:{reindex}");
            diagnostics.WriteLine($"- trackTokens: {timerTrackTokens.TotalMilliseconds:N}");
            diagnostics.WriteLine($"- updateContext: {(timerUpdateContext.TotalMilliseconds - timerTrackTokens.TotalMilliseconds):N}");
            string writerMsg = writer.ToString();
            if (!string.IsNullOrWhiteSpace(writerMsg))
                diagnostics.WriteLineNoTabs(writerMsg);
            diagnostics.WriteLine($"- other: {(sw.Elapsed.TotalMilliseconds - timerUpdateContext.TotalMilliseconds):N}");
        }
