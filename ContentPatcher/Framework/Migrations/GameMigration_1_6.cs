...
{
    internal class GameMigration_1_6 : GameMigration
    {
        ...
        private const string warning = "Some content packs haven't been updated for Content Patcher 2.0.0 and Stardew Valley 1.6.0. Content Patcher will try to auto-migrate them, but compatibility isn't guaranteed.";
        public GameMigration_1_6()
            : base(new SemanticVersion(1, 6, 0), warning, new SemanticVersion(2, 0, 0))
        {
        }
        ...
        *********/
        ...
        {
            if (assetName.IsEquivalentTo("Data/NPCDispositions"))
            {
                return new AssetName("Data/Characters", assetName.LocaleCode, assetName.LanguageCode);
            }
            else if (assetName.IsEquivalentTo("Data/ObjectInformation"))
            {
                return new AssetName("Data/Objects", assetName.LocaleCode, assetName.LanguageCode);
            }
            else if (assetName.IsEquivalentTo("Data/BigCraftableInformation"))
            {
                return new AssetName("Data/BigCraftables", assetName.LocaleCode, assetName.LanguageCode);
            }
        ...
        /// <inheritdoc />
        public override bool TryApplyLoadPatch(LoadPatch patch, IAssetName assetName, [NotNullWhen(true)] out object? asset, out string? error)
        {
            asset = null;
            error = null;
            return false;
        ...
        /// <inheritdoc />
        public override bool TryApplyEditPatch(IPatch patch, IAssetName assetName, object asset, out string? error)
        {
            error = null;
            return false;
        }
            ...
            foreach (PatchConfig? patch in patches)
            {
                if (this.HasAction(patch, PatchType.EditData))
                {
                    ...
                    else if (PathUtilities.NormalizeAssetName(patch.Target) == "Data/Locations")
                    {
                        ...
                        {
                            // No errors mapping, now to convert Entries to Fields to prevent deleting CreateOnLoad.
                            // Only doing so when Fields don't overlap to simplify the problem
                            foreach (var entry in patch.Entries)
                            {
                                if (entry.Value is JObject objectValue && !patch.Fields.TryGetValue(entry.Key, out _))
                                {
                                    patch.Fields[entry.Key] = new InnerPatch(objectValue);
                                    patch.Entries.Remove(entry.Key);
                                }
                            }
                        }
